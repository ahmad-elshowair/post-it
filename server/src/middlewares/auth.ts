...
const authorizeUser = (req: ICustomRequest, res: Response, next: NextFunction) => {
  try {
    // Get token from cookie first (preferred) or from Authorization header if not in cookie.
    const accessTokenName =
      config.node_env === 'development' ? 'access_token' : '__Host-access_token';
    ...
    if (!token) {
      console.error(` No token found in the cookies (${accessTokenName}) or Authorization header`);
    ...
    const fingerprintName =
      config.node_env === 'development' ? 'x-fingerprint' : '__Host-x-fingerprint';
    ...
    if (decodedUser.fingerprint && fingerprint) {
      ...
      if (hashedFingerprint !== decodedUser.fingerprint) {
        console.warn(`Potential token theft detected for user ID: ${decodedUser.id}`);
        return res.status(403).json({
          message: 'Authentication Failed',
    ...
    if (config.csrf_protection_enabled) {
      ...
      const csrfTokenName =
        config.node_env === 'development' ? 'csrf_token' : '__Secure-csrf_token';

      const storedCsrfToken = req.cookies[csrfTokenName];

      // PROPER VALIDATION OF TOKEN PRESENCE AND FORMAT.
      if (!csrfToken || typeof csrfToken !== 'string') {
        console.error('CSRF validation failed - missing or invalid format in header');
        return res.status(403).json({
          message: 'Authentication failed',
