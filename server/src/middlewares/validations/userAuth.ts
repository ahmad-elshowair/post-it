...
export const checkLength = (field: string, min: number, message: string): ValidationChain =>
  check(field).isLength({ min }).withMessage(message);
  ...
  async (password: string, { req }) => {
    ...
    try {
      ...
      if (error instanceof Error && error.message !== ' EMAIL DOES NOT EXIST !') {
        throw new Error(`Check Password: ${(error as Error).message}!`, { cause: error });
      }
    } finally {
      connection.release();
    }
