...
export const getFingerprintFromCookie = (): string | null => {
  const cookieName = configs.node_env === 'development' ? 'x-fingerprint' : '__Host-x-fingerprint';
...
export const getCsrfFromCookie = (): string | null => {
  const cookieName = configs.node_env === 'development' ? 'csrf_token' : '__Secure-csrf_token';
  const cookies = document.cookie.split('; ');
  for (let cookie of cookies) {
    const [name, value] = cookie.trim().split('=');
    if (name === cookieName) {
...
export const syncAllAuthTokensFromCookies = () => {
  const cookieFingerprint = getFingerprintFromCookie();
  const localFingerprint = getFingerprint();
  const fingerprintSynced = cookieFingerprint && cookieFingerprint !== localFingerprint;
