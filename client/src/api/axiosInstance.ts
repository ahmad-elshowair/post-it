...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (status === 401 && !originalRequest._retry && originalRequest.url !== refreshTokenUrl) {
      ...
      try {
        ...
        if (!fingerprint) {
          console.warn('Axios Interceptor: No fingerprint found for refresh request. Logging out.');
        ...
        const { data } = await api.post(refreshTokenUrl, {}, { withCredentials: true, headers });
    ...
    if (status === 401 && originalRequest.url === refreshTokenUrl) {
      ...
      useAuthStore.getState().logout();
      console.log('Axios Interceptor: Redirecting to /login due to refresh token 401.');
      window.location.href = '/login';
